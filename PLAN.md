...
> Estoy reviviendo un sistema de gestión de una financiera. Repo: https://github.com/matiaslujanw/financiera-digital (cloná si hace falta). La app está en `financiera/` (Next.js). Setup: `cd financiera && npm install`, después `npm run dev` (levanta pg-server en :5433 + Next en :3000). **Antes de programar, leé `PLAN.md` en la raíz de punta a punta** — tiene el roadmap, el estado y las decisiones técnicas. Estado: Fases 0 a 4 completas. **En curso (rama `rediseno-dorado-neon`):** rediseño UI **dorado neón sobre negro** (ver sección *Transversal*) y la pantalla **Transacciones** revivida como home (Crear transacción arriba, tabla de movimientos, Resumen colapsable), con un **creador inline de doble entrada** ya funcionando. **Próximo:** paso 2 del creador inline (campos **Categoría + Subcuenta**, requieren endpoints de catálogo), terminar el diseño en las pantallas/diálogos restantes, y el **selector de cuenta en compra/venta de cheques**; después la Fase 5 (préstamos/créditos). La base local arranca vacía en una compu nueva (registrá un negocio en `/register`).
